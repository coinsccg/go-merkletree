...
// MerkleProof Get Merkle path and indexes(left leaf or right leaf)
func (m *MerkleTree) MerkleProof(content Content) ([][]byte, []int64, error) {
...
func buildIntermediate(nl []*Node, t *MerkleTree) (*Node, error) {
	...
	for i := 0; i < len(nl); i += 2 {
		...
		leftByte := nl[left].Hash
		rightByte := nl[right].Hash
		var chash []byte
		if bytes.Compare(leftByte, rightByte) < 0 {
			chash = append(leftByte, rightByte...)
		} else {
			chash = append(rightByte, leftByte...)
		}
